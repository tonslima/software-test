...
Ao submeter um arquivo para reconciliação, o cliente **deve** informar a `referenceDate` correspondente ao arquivo — o **dia de liquidação ao qual o CSV se refere** (o "dia do PaySettler").
...
- **Exemplo:** `referenceDate=2025-03-20` para reconciliar o arquivo do dia 20/03/2025.

Para a semântica completa (janela de 7 dias, persistência, validações), veja `domain-glossary.md → Escopo temporal`.
...
- Transações com status `REVERSED` também aparecem no arquivo e **não devem ser ignoradas** — elas participam do processo de reconciliação normalmente.
